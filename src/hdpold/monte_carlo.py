...
class MonteCarlo(object):
    ...
    def _initialize(self,
        ...
        # initialize alpha_alpha, the smoothing value for a word to be assigned to a new table
        # the probability of f_{k_{\mathsf{new}}}^{-x_{dv}}(x_{dv}), the prior probability density for x_{dv}
        self._alpha = alpha_alpha;
        # initialize alpha_eta, the smoothing value for a word to be assigned to a new topic
        self._alpha_eta = alpha_eta;
        # initialize alpha_gamma, the smoothing value for a table to be assigned to a new topic
        ...
        for iter in xrange(iteration):
            for document_index in numpy.random.permutation(xrange(self._D)):
                ...
                for word_index in numpy.random.permutation(xrange(len(self._corpus[document_index]))):
                    ...
                    f_new = self._alpha_gamma / self._V;
                    for k in xrange(self._K):
                        f[k] = (self._n_kv[k, word_id] + self._alpha_eta) / (n_k[k] + self._V * self._alpha_eta);
                        f_new += self._m_k[k] * f[k];
                    f_new /= (numpy.sum(self._m_k) + self._alpha_gamma);
                    ...
                    if new_table == len(self._k_dt[document_index]):
                        ...
                        topic_probability[self._K] = self._alpha_gamma / self._V;
                ...
                for table_index in numpy.random.permutation(xrange(len(self._k_dt[document_index]))):
                    ...
                    if self._n_dt[document_index][table_index] > 0:
                        ...
                        topic_probability = numpy.zeros(self._K + 1);
                        topic_probability[self._K] = scipy.special.gammaln(self._V * self._alpha_eta) - scipy.special.gammaln(self._n_dt[document_index][table_index] + self._V * self._alpha_eta);
                        ...
                        topic_probability[self._K] += numpy.log(self._alpha_gamma);
                        ...
                        for topic_index in xrange(self._K):
                            if topic_index == old_topic:
                                if self._m_k[topic_index] <= 1:
                                    ...
                                    topic_probability[topic_index] = scipy.special.gammaln(self._V * self._alpha_eta + n_k[topic_index] - self._n_dt[document_index][table_index]) - scipy.special.gammaln(self._V * self._alpha_eta + n_k[topic_index]);
                                    for word_id in selected_word_freq_dist.keys():
                                ...
                                topic_probability[topic_index] = scipy.special.gammaln(self._V * self._alpha_eta + n_k[topic_index]) - scipy.special.gammaln(self._V * self._alpha_eta + n_k[topic_index] + self._n_dt[document_index][table_index]);
                                for word_id in selected_word_freq_dist.keys():
    ...
    def topic_log_likelihood(self):
        log_likelihood = self._K * numpy.log(self._alpha_gamma) - log_factorial(numpy.sum(self._m_k), self._alpha_gamma);
        for topic_index in xrange(self._K):
    ...
    def word_log_likelihood(self):
        ...
        log_likelihood = self._K * scipy.special.gammaln(self._V * self._alpha_eta);
        for topic_index in xrange(self._K):
            log_likelihood -= scipy.special.gammaln(self._V * self._alpha_eta + n_k[topic_index]);
    ...
    def export_snapshot(self, directory, index):
        ...
        hyper_parameter = numpy.array([self._alpha, self._alpha_eta, self._alpha_gamma]);
