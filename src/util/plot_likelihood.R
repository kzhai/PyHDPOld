...
true_input_data <- subset(input_data, input_data$inference=='true')$likelihood;
...
plot_pic <- ggplot() +
	...
	geom_smooth(data=input_data, aes(x=iteration, y=likelihood, color=inference, group=inference), alpha=0.75, size=1) +
